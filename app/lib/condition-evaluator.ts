 ...
 * @param formValues - A record of field IDs/names to their current values
 * @returns true if the element should be visible, false if hidden
 */
...
/** Loose equality: coerce to string for comparison so "5" == 5, true == "true", etc. */
...
/** Get operators applicable to a given field data type */
export function getOperatorsForType(dataType: string): OperatorInfo[] {
  ...
  switch (dt) {
    ...
    case "CHECKBOX_GROUP":
      ...
      break;
    default:
      // TEXT, TEXTAREA, EMAIL, PHONE, URL, etc.
