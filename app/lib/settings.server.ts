...
export const SETTING_DEFAULTS: Record<string, { value: string; type: string; category: string }> = {
  ...
  "email.from_name": { value: "Events Platform", type: "string", category: "email" },
  "general.app_name": { value: "Events Platform", type: "string", category: "general" },
  "general.default_timezone": { value: "UTC", type: "string", category: "general" },
  "workflow.auto_assign": { value: "false", type: "boolean", category: "workflow" },
};
...
/**
 * Get a setting value with hierarchical scope resolution.
 * Resolution order: user > event > tenant > global > hardcoded default.
 */
export async function getSetting(
  ...
  if (context?.eventId) {
    scopeFilters.push({ scope: "event", scopeId: context.eventId });
  }
  if (context?.userId) {
...
/**
 * Upsert a setting at a given scope.
 */
...
/**
 * Get all settings for a category, with resolved values for the given context.
 */
export async function getSettingsByCategory(
  ...
  // Build a map of key -> settings at various scopes
  ...
  // Also include defaults for this category that aren't in DB
...
/**
 * Get all settings across all categories (for the admin overview).
 */
...
/**
 * Delete a setting override at a specific scope.
 */
