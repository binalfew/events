...
sequenceDiagram
    ...
    DSE->>P: Load FieldDefinitions for event
...
| **Field Definitions** (FieldDefinition for an event)               | Cache on first access per event    | 10 minutes              | Event-driven: `fieldDef.updated` event clears cache for event |
