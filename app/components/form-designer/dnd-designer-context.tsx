...
// We prefix IDs so we can distinguish between sections, fields, and palette items
...
export interface ActiveDragInfo {
  type: "section" | "field" | "palette";
  id: string;
  /** For field/palette: the label to display in the overlay */
  label?: string;
  /** For field/palette: the data type for icon */
  dataType?: string;
  /** For field: its current colSpan */
  colSpan?: number;
}
...
// Use pointerWithin for cross-container detection, fallback to rectIntersection,
// then closestCenter for fine-grained positioning
...
export function DndDesignerContext({
  ...
  /** Find which section a field belongs to on the active page */
    ...
    (event: DragEndEvent) => {
      ...
      if (activeParsed.type === "field") {
        ...
        if (overParsed.type === "field") {
          // Dropping on another field — insert at that field's position
