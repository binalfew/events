...
export const fieldNameSchema = z
  ...
  .max(
    CUSTOM_FIELD_LIMITS.maxNameLength,
    `Name must be at most ${CUSTOM_FIELD_LIMITS.maxNameLength} characters`,
  )
  .regex(
    /^[a-z][a-z0-9_]*$/,
...
export const createCustomFieldSchema = z.object({
  ...
  label: z
    ...
    .max(
      CUSTOM_FIELD_LIMITS.maxLabelLength,
      `Label must be at most ${CUSTOM_FIELD_LIMITS.maxLabelLength} characters`,
    ),
  description: z
    .string()
    .max(
      CUSTOM_FIELD_LIMITS.maxDescriptionLength,
      `Description must be at most ${CUSTOM_FIELD_LIMITS.maxDescriptionLength} characters`,
    )
    .optional(),
...
export const updateCustomFieldSchema = createCustomFieldSchema.omit({ eventId: true }).partial();
