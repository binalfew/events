...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  try {
    if (_action === "create") {
      ...
      return redirect(`/admin/events/${eventId}/forms/${template.id}`);
    }
    ...
    if (_action === "clone") {
      ...
      return redirect(`/admin/events/${eventId}/forms`);
    }
              ...
              {templates.map((template) => (
                <tr key={template.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      to={`/admin/events/${event.id}/forms/${template.id}`}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Link to={`/admin/events/${event.id}/forms/${template.id}`}>
