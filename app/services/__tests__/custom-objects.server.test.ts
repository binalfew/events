...
describe("custom-objects.server", () => {
  ...
  describe("createDefinition", () => {
    it("creates a definition with valid slug", async () => {
      const { createDefinition } = await import("../custom-objects.server");
    ...
    it("throws for invalid slug", async () => {
      const { createDefinition, CustomObjectError } = await import("../custom-objects.server");
  ...
  describe("deleteDefinition", () => {
    it("deletes a definition with no records", async () => {
      ...
      expect(mockDefDelete).toHaveBeenCalledWith({ where: { id: "def-1" } });
    });
  ...
  describe("listDefinitions", () => {
    it("lists active definitions by default", async () => {
      const { listDefinitions } = await import("../custom-objects.server");
  ...
  describe("updateRecord", () => {
    it("updates a record", async () => {
      const { updateRecord } = await import("../custom-objects.server");
      mockRecFindUniqueOrThrow.mockResolvedValue({
        id: "rec-1",
        definition: {
          fields: [{ name: "plate", label: "License Plate", dataType: "TEXT", required: false }],
        },
      });
      mockRecUpdate.mockResolvedValue({ id: "rec-1", data: { plate: "XYZ-999" } });

      const result = await updateRecord("rec-1", { data: { plate: "XYZ-999" } });

      expect(result.data).toEqual({ plate: "XYZ-999" });
    });

    it("throws when required field missing on update", async () => {
      const { updateRecord, CustomObjectError } = await import("../custom-objects.server");
      mockRecFindUniqueOrThrow.mockResolvedValue({
        id: "rec-1",
        definition: {
          fields: [{ name: "name", label: "Name", dataType: "TEXT", required: true }],
        },
      });

      await expect(updateRecord("rec-1", { data: {} })).rejects.toThrow(CustomObjectError);
    });
  });
  ...
  describe("listRecords", () => {
    it("lists records for a definition", async () => {
      const { listRecords } = await import("../custom-objects.server");
