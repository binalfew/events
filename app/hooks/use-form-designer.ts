...
export function designerReducer(state: DesignerState, action: DesignerAction): DesignerState {
  switch (action.type) {
    ...
    case "REORDER_PAGES": {
      ...
      if (
        fromIndex < 0 ||
        fromIndex >= def.pages.length ||
        toIndex < 0 ||
        toIndex >= def.pages.length ||
        fromIndex === toIndex
      ) {
        return state;
      }
...
// ─── Mutating action types (push to undo stack) ─────────
...
export function useFormDesigner(initialDefinition: FormDefinition | undefined | null) {
  ...
  const setViewMode = useCallback(
    (viewMode: DesignerState["viewMode"]) => {
  ...
  const updatePage = useCallback(
    (
      pageId: string,
      updates: Partial<Omit<FormDefinition["pages"][number], "id" | "sections">>,
    ) => {
  ...
  const updateSection = useCallback(
    (
      pageId: string,
      sectionId: string,
      updates: Partial<Omit<FormDefinition["pages"][number]["sections"][number], "id" | "fields">>,
    ) => {
  ...
  const addField = useCallback(
    (
      pageId: string,
      sectionId: string,
      field: FormDefinition["pages"][number]["sections"][number]["fields"][number],
    ) => {
  ...
  const removeField = useCallback(
    (pageId: string, sectionId: string, fieldId: string) => {
  ...
  const updateField = useCallback(
    (
      ...
      fieldId: string,
      updates: Partial<
        Omit<FormDefinition["pages"][number]["sections"][number]["fields"][number], "id">
      >,
    ) => {
  ...
  const updateSettings = useCallback(
    (settings: Partial<NonNullable<FormDefinition["settings"]>>) => {
