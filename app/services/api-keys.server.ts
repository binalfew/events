...
export async function createApiKey(input: CreateApiKeyInput, ctx: ServiceContext) {
...
export async function updateApiKey(id: string, input: UpdateApiKeyInput, ctx: ServiceContext) {
...
export async function revokeApiKey(id: string, ctx: ServiceContext) {
  const existing = await prisma.apiKey.findFirst({
    where: { id, tenantId: ctx.tenantId },
  });
  if (!existing) {
...
export async function rotateApiKey(id: string, gracePeriodHours: number, ctx: ServiceContext) {
  const existing = await prisma.apiKey.findFirst({
    where: { id, tenantId: ctx.tenantId },
  });
  if (!existing) {
  ...
  // Mark old key as ROTATED with grace period, create new key
...
export async function validateApiKey(rawKey: string): Promise<ValidatedApiKey | null> {
  ...
  // Find candidates by prefix (typically 1)
