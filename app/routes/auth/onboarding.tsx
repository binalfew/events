...
export async function action({ request }: Route.ActionArgs) {
  ...
  // Create user + password + assign default USER role atomically
  ...
  const user = await prisma.$transaction(async (tx) => {
    ...
    // Find the default USER role (any tenant's EVENT-scoped USER role)
    const userRole = await tx.role.findFirst({
      where: { name: "USER", scope: "EVENT" },
    });
    if (userRole) {
              ...
              <Form method="post" {...getFormProps(form)}>
                <div className="flex flex-col gap-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    {(() => {
                      const { key, ...props } = getInputProps(fields.username, {
                        type: "text",
                      });
                      return (
                        <Input
                  ...
                  <div className="grid gap-2">
                    ...
                    {(() => {
                      const { key, ...props } = getInputProps(fields.name, {
                        type: "text",
                      });
                      return (
