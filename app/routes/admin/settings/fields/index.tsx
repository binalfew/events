...
export const handle = { breadcrumb: "Global Fields" };
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    if (_action === "delete") {
      ...
      return redirect("/admin/settings/fields");
    }
    ...
    if (_action === "reorder") {
      ...
      const fields = await listFields(tenantId, { scope: "global" });
...
export default function GlobalFieldsListPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Global Fields</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Define fields that are available across all events. {fields.length} global field
            {fields.length !== 1 ? "s" : ""} defined.
          </p>
        </div>
        <Link to="/admin/settings/fields/new">
          <Button>Add Global Field</Button>
        </Link>
      </div>
