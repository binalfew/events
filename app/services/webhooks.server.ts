...
export async function createWebhookSubscription(input: CreateWebhookInput, ctx: ServiceContext) {
...
export async function deleteWebhookSubscription(id: string, ctx: ServiceContext) {
  const existing = await prisma.webhookSubscription.findFirst({
    where: { id, tenantId: ctx.tenantId },
  });
  if (!existing) {
...
export async function pauseWebhookSubscription(id: string, ctx: ServiceContext) {
  const existing = await prisma.webhookSubscription.findFirst({
    where: { id, tenantId: ctx.tenantId },
  });
  if (!existing) {
...
export async function resumeWebhookSubscription(id: string, ctx: ServiceContext) {
  const existing = await prisma.webhookSubscription.findFirst({
    where: { id, tenantId: ctx.tenantId },
  });
  if (!existing) {
...
export async function testWebhookEndpoint(id: string, ctx: ServiceContext) {
  const existing = await prisma.webhookSubscription.findFirst({
    where: { id, tenantId: ctx.tenantId },
  });
  if (!existing) {
